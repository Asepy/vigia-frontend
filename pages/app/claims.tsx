...
const AppQuestions: NextPage = () => {
  ...
  const [pruebatexto, setpruebatexto] = React.useState("");
  ...
  var fieldsGroups: Array<any> = [
    ...
    },
    {
      ...
      type: "select",
      options: [
        { value: "planning", name: "Planificación" },
    ...
    type:'select',
    options:[{value:'1',name:'Pendiente'}]
  }*/
    {
      name: "Estado del reclamo",
      id: "taskStatus",
      group: ["taskStatusL"],
      type: "text",
    },
  ...
  function applyFilter(field: string, value?: any, defineValue?: any) {
    ...
    router.push(`/app/claims${getFiltersString(parameters)}`);
  }
  ...
  async function getMyClaims() {
    ...
    try {
      const data = await fetchData("getMyClaims",filters,"POST",true);
      if (!data.error) {
        if (data?.data?.length) {
        ...
        setMessage("No tienes ninguna reclamo todavia");
        setOpenMessage(true);
        setQuestions([]);
      }
    } catch (error) {
      setMessage("Aun no tienes reclamos");
      setOpenMessage(true);
      console.dir(error);
      setQuestions([]);
    } finally {
  ...
  return (
    <>
      <Head>
        <title>VigiA - Mis Reclamos</title>
        <meta name="description" content="Mis Reclamos" />
                ...
                <Grid item xs={12} sm={7} md={7} lg={7} xl={7}>
                  <ThemeProvider theme={PersonalizedTextSearchWhite}>
                    <TextField
                      ...
                      label="Buscar contenido del reclamo..."
                      name="search"
                      type="text"
                      variant="filled"
                      fullWidth
                        ...
                        Questions.map((Question: any, index: number) => {
                          return (
                            <tr key={index} title={
                              ...
                              <td data-label="Fecha">
                                {getDateFormat(Question.fecha_creacion)}
                              </td>
                              <td data-label="Llamado">
                                <Link
                                  href={
                                    "/identifiedProcess?id=" +
                                    encodeURIComponent(Question.llamado) +
                                    "&state=claim"
                                  }
                                >
                              ...
                              <td data-label="Etapa del Proceso">
                                {getStage(Question.etapa)}
                              </td>
                              <td data-label="Estado del Reclamo">
                                {Question.tarea_descripcion}
                              </td>
                              {/*
                              ...
                              <td data-label="">
                                <Link
                                  href={
                                    "/claims/claim?id=" +
                                    encodeURIComponent(Question.enlace)
                                  }
                                >
                                  <Button
                                    title="Visualizar"
                      ...
                      {PaginationState.pages.map((page: any, index: number) => {
                        return page == "..." ? (
                          ...
                          <Link
                            key={index}
                            href={
                              "/app/claims" + getFiltersString({ page: page })
                            }
                          >
                            <div
                              className={
