...
const AppQuestions: NextPage = () => {
  ...
  function applyFilter(field: string, value?: any, defineValue?: any) {
    ...
    router.push(`/app/questions${getFiltersString(parameters)}`);
  }
  ...
  async function getMyQuestions() {
    ...
    try {
      ...
      const data = await fetchData("getMyQuestions",filters,"POST",true); 
      if (!data.error) {
        if (data?.data?.length) {
  ...
  return (
    <>
      <Head>
        <title>VigiA - Mis Consultas</title>
        <meta name="description" content="Mis Consultas" />
